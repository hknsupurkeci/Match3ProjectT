...
public class BoardCreator : MonoBehaviour
{
    ...
    private void CheckMatches()
    {
        ...
        // Yatayda ve dikeyde taş eşleşmelerini kontrol et
                ...
                if (shape[x, y] == 1 && currentObj != null)
                {
                    ...
                    if (y < boardWidth - 2)
                    {
                        ...
                        if (obj1 != null && obj2 != null)
                        {
                            ...
                            {
                                Debug.Log("Yatay eşleşme bulundu: " + currentObj.tag);
                                // Eşleşme bulunduğunda yapılacak işlemleri burada gerçekleştirin
                    ...
                    if (x < boardHeight - 2)
                    {
                        ...
                        if (obj1 != null && obj2 != null)
                        {
                            if (currentObj.tag == obj1.tag && currentObj.tag == obj2.tag)
                            {
                                Debug.Log("Dikey eşleşme bulundu: " + currentObj.tag);
                                // Eşleşme bulunduğunda yapılacak işlemleri burada gerçekleştirin
                                StartCoroutine(FillBoardCoroutine(currentObj, obj1, obj2));
                            }
                        }
    ...
    private IEnumerator FillBoardCoroutine(GameObject currentObj, GameObject obj1, GameObject obj2)
    {
        ...
        // Pozisyonları kontrol edin ve saklayın
        ...
        for (int x = 0; x < boardHeight; x++)
        {
            for (int y = 0; y < boardWidth; y++)
            {
                ...
                {
                    //Prefabs
                    Vector3 hücreKonumu2 = new Vector3(xValue, yValue, -0.1f);
        ...
        for (int x = 0; x < boardHeight; x++)
        {
            for (int y = 0; y < boardWidth; y++)
            {
                if (shape[x, y] == 1)
                {
                    ...
                    // Lerp hareketi için coroutine başlat
    ...
    /// <summary>
    /// Akici bir sekilde preflerin baslangıc yerlerine gelmesini saglar.
    /// </summary>
    ...
    IEnumerator MoveTileToPosition(GameObject tile, Vector3 targetPosition)
    {
        ...
        float lerpTime = 2f; // Bu süreyi ayarlayarak hareketin ne kadar süre alacağını belirleyebilirsiniz.
    ...
    private GameObject GetValidTile(int x, int y)
    {
        ...
        return tilePrefabs[Random.Range(0, tilePrefabs.Length)]; // Fallback, bu durumun olmaması gerekir.
    }
    ...
    IEnumerator LerpPosition(Vector3 startPos, Vector3 targetPos, GameObject obj)
    {
        ...
        // İşlem sonrası pozisyonu kesinleştirin
